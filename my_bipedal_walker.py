...
class ContactDetector(contactListener):
    ...
    def BeginContact(self, contact):
        if (
            ...
            hull_pos = self.env.hull.position
            top_y = hull_pos[1] + 10 / SCALE  # Top edge (relative to center)
            bottom_y = hull_pos[1] - 10 / SCALE  # Bottom edge
            left_x = hull_pos[0] - 30 / SCALE  # Left edge
            right_x = hull_pos[0] + 30 / SCALE  # Right edge

            # Check if collision is on the top of the hull
            if (
                contact_point[1] > top_y  # Above the top
                and left_x <= contact_point[0] <= right_x  # Within horizontal bounds
            ):
                self.game_over = True
                return
...
class BipedalWalkerWithObject(gym.Env, EzPickle):
    """
    ## Description
    ...
    def __init__(self, render_mode: Optional[str] = None, hardcore: bool = False):
        ...
        low = np.append(low, [-np.inf, -np.inf])  # Extend with bounds for object position
    ...
    def reset(
        ...
        *,
        ...
        self.object = self.world.CreateDynamicBody(
            position=(self.hull.position[0], self.hull.position[1] + 0.5),  # Adjust height above the hull
            fixtures=fixtureDef(
                shape=polygonShape(box=(0.5, 0.5)),  # Cube with width and height of 1
                density=1.0,  # Affects weight and inertia
                friction=0.6,  # Determines sliding behavior
                restitution=0.0,  # No bounciness
            ),
        )
        self.object.color1 = (255, 0, 0)  # Red for visualization
        self.object.color2 = (200, 0, 0)
        ...
        friction=0.3,
        restitution=0.0,
        categoryBits=0x0020,  # Matches the hull's category
        maskBits=0x001,       # Ensures it interacts with the hull
        )
        ...
        self.drawlist = self.terrain + self.legs + [self.hull, self.object]
    ...
    def step(self, action: np.ndarray):
        ...
        object_pos = self.object.position
        ...
        reward = 0
        reward = pos[0]  # Reward forward progress
        ...
        for a in action:
            ...
            #reward = 0.1 * pos[0]  # Reward forward progress

            # normalized to about -50.0 using heuristic, more optimal agent should spend less

        terminated = False
        if self.game_over or pos[0] < 0 or object_pos[1] < 0.5:
            reward -= 100
            terminated = True
        if abs(object_pos[0] - pos[0]) > 1.5 or object_pos[1] < pos[1] - 1:
            reward -= 100  # Penalty for dropping the object
        ...
        if self.render_mode == "human":
            self.render()
        state += [object_pos[0] - pos[0], object_pos[1] - pos[1]]
