...
def discretizeState(state):
    ...
    for i in range(len(state)):
        #print(state[i])
        #print(i)
        index = int((state[i]-stateBounds[i][0]) / (stateBounds[i][1]-stateBounds[i][0])*9)
...
def convertNextAction(nextAction):
    return [action / 9 * 2 - 1 for action in nextAction]  
...
def runAlgorithmStep(env, i, doRender):
    ...
    print("Episode #: ", i)
    ALPHA = max(0.01, 0.1 * (1 - i / EPISODES))
...
def main():
    ...
    qTable_serializable = {str(key): value.tolist() for key, value in qTable.items()}
